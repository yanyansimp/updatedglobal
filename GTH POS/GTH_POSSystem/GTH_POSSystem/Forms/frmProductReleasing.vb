Public Class frmProductReleasing
...
exit_loop2:
    ...
    Private Sub LoadPOToCombo(ByVal suppid As Long)
        ...
        With dsDataset.Tables("a")
            If .Rows.Count > 0 Then
                cmbDRNumber.Items.Clear()
                cmbDRNumber.Text = ""
                For rowcounter = 0 To .Rows.Count - 1
                    cmbDRNumber.Items.Add(New ComboListData(.Rows(rowcounter).Item("PO_Number").ToString().PadLeft(6, "0"), .Rows(rowcounter).Item("trnno").ToString()))
                Next
                cmbDRNumber.SelectedIndex = 0
            End If
        End With
        sqladapter.Dispose()
        dsDataset.Dispose()
    ...
    Private Sub LoadSuppliersToCombo()
        ...
        With dsDataset.Tables("a")
            If .Rows.Count > 0 Then
                ...
                For rowcounter = 0 To .Rows.Count - 1
                    cmbCustomerName.Items.Add(New ComboListData(.Rows(rowcounter).Item("SupplierName").ToString(), .Rows(rowcounter).Item("SupplierID").ToString()))
                Next
            End If
        End With
        sqladapter.Dispose()
    ...
    Private Sub LoadCustomersToCombo()
        Dim rowcounter As Integer
        sqladapter = New SqlClient.SqlDataAdapter("Select   trnno, CustomerName from tblcustomer order by CustomerName", conn)
        dsDataset = New DataSet
        dsDataset.Reset()
        sqladapter.Fill(dsDataset, "a")
        With dsDataset.Tables("a")
            If .Rows.Count > 0 Then
                cmbCustomerName.Items.Clear()
                For rowcounter = 0 To .Rows.Count - 1
                    cmbCustomerName.Items.Add(New ComboListData(.Rows(rowcounter).Item("CustomerName").ToString(), .Rows(rowcounter).Item("trnno").ToString()))
                Next
            End If
        End With
        sqladapter.Dispose()
    ...
    Private Sub getCustomerIDFromComboBOx()
        Try
            For Each item As ComboListData In cmbCustomerName.Items
                If item.Name.ToString = cmbCustomerName.SelectedItem.ToString Then
                    CustomerID = item.ItemData.ToString
                    GoTo exit_loop2
                End If
            Next
        Catch
...
exit_loop2:
    ...
    Private Sub getSupplierIDFromComboBOx_____()
        Try
            For Each item As ComboListData In cmbCustomerName.Items
                If item.Name.ToString = cmbCustomerName.SelectedItem.ToString Then
...
exit_loop2:
    ...
    Private Sub load_PO_details(ByVal PONUM As String)
        ...
        sqladapter = New SqlClient.SqlDataAdapter("SELECT     ProductID_Barcode as Barcode, Description, QtyOrdered as Ordered, UNIT as Unit,PO_Number,PurchaseCost " _
                                        & " FROM vwPO  WHERE      PO_Number  = '" & cmbDRNumber.Text & "'", conn)
        dsDataset = New DataSet
        dsDataset.Reset()
        sqladapter.Fill(dsDataset, "a")
    ...
    Private Sub btnNew_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnNew.Click
        If btnNew.Text = "&New" Then
            'get the new releasing ref no (highest trnno + 1 in tblStocksReleasingSummary)
            Dim SRRefNo As Integer = getFieldMaxValue("tblStocksReleasingSummary", "trnno") + 1

            Call change_bkGround(Color.White)
            Call clearControls()
            'get the new batch number for this batch of products received
            txtDRNumber.Text = SRRefNo
            cmbDRNumber.Text = txtDRNumber.Text.PadLeft(10, "0")

            Call LoadCustomersToCombo()
    ...
    Private Sub btnSave_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnSave.Click
        ...
        Dim DrRefNo As String = txtDRNumber.Text
        ...
        If Len(txtDRNumber.Text) <= 0 Then
            ...
            txtDRNumber.Focus()
            Exit Sub
        ElseIf Len(cmbCustomerName.Text) <= 0 Then
            MsgBox("INVALID ENTRY, PLEASE SELECT CUSTOMER FROM CUSTOMER LIST. THANK YOU", MsgBoxStyle.Critical, "SAVING ERROR. . .")
            cmbCustomerName.Focus()
            Exit Sub
        End If

        Call getCustomerIDFromComboBOx()
        ...
        If btnSave.Text = "&Update" Then
            ...
            If MsgBox("Do you want to EDIT more entries for this Batch?", MsgBoxStyle.YesNo + MsgBoxStyle.Exclamation, "EDIT MORE ENTRIES?") = MsgBoxResult.Yes Then
                Call clearControls()
                Call change_bkGround(Color.White)
        ...
        Else
            With DataGridView2
                Dim SRRefNo As Integer = getFieldMaxValue("tblStocksReleasing", "SRREFNO") + 1
                ...
                For xDatagridRows As Integer = 0 To .Rows.Count - 2
                    ...
                    qtyReceived = IIf(.Rows(xDatagridRows).Cells(2).Value = "", 0, .Rows(xDatagridRows).Cells(2).Value)
                    Dim QTYONHAND As Long = getFIELD_VALUE_FROM_DBASE("tblProduct", "QtyOnHand", "ProductID_Barcode", xxyzz)
                    Dim newQtyOnHand As Long = QTYONHAND - evaluateQTY_to_Release(xxyzz)
                    Dim strUpdateTblProduct_VALUES As String = " QtyOnHand=" & newQtyOnHand & ", QtyOut = " & qtyReceived & ""
                    ...
                    'Dim BatchNum As String = .Rows(xDatagridRows).Cells(8).Value
                    'Dim strDelivery_status As String = .Rows(xDatagridRows).Cells(6).Value
                    'Dim rems As String = .Rows(xDatagridRows).Cells(7).Value


                    TotalAmount = TotalAmount + (.Rows(xDatagridRows).Cells(2).Value * .Rows(xDatagridRows).Cells(4).Value)
                    'purchasecost = .Rows(xDatagridRows).Cells(5).Value

                    'Insert to tblProductReceived Table 

                    Dim strVALUES As String = "'" & Replace(xxyzz, "'", "''") & "','" & DateTimePicker1.Text & "','" & qtyReceived & "'," & USER_ID_LOGGED & "," & SupplierID & ",'" & timeofdAY & "'," & Batch_Number & ",'" & DrRefNo & "','" & purchasecost & "'"
                    Call INSERT_DATA_TO_DATABASE("tblStocksReleasing", "SRRefNo,ProductID_Barcode,QuantityReleased,DiscountValue,Amount, TransactionTime", strVALUES)
                ...
                Else
                    addtopayable = 0
                End If
                Dim strVALUES2 As String = "'" & DrRefNo & "','" & DateTimePicker1.Text & "','" & SupplierID & "'," & Batch_Number & ",'" & TotalAmount & "','" & addtopayable & "'"
                Call INSERT_DATA_TO_DATABASE("tblProductReceivedSummary", "DR_RefNo,TransactionDate,SupplierID,BatchNumber,DR_TotalAmount,AddedToAccountPayable", strVALUES2)
        ...
        '
        'LOAD PRODUCT RECEIVED with THE FOLLOWING SORT"
    ...
    Private Sub frmProductReleasing_FormClosed(ByVal sender As Object, ByVal e As System.Windows.Forms.FormClosedEventArgs) Handles Me.FormClosed
        frmMain.TStripStocksReleasing.Enabled = True
    End Sub
    ...
    Private Sub btnAddNewSupplier_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnDisplayPO.Click
        If DATA_WAS_EDITED = True Then
            If MsgBox("CHANGES NOT SAVED, RELOAD P.O ANYWAY?", MsgBoxStyle.YesNo + MsgBoxStyle.Critical, "") = MsgBoxResult.Yes Then
                Call load_PO_details(cmbDRNumber.Text)
                DATA_WAS_EDITED = False
            End If
        Else
            Call load_PO_details(cmbDRNumber.Text)
    ...
    Private Sub ToolStripMenuItem1_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles ToolStripMenuItem1.Click
        ...
        With DataGridView2
            ...
            cmbCustomerName.Text = getFIELD_VALUE_FROM_DBASE("tblSupplier", "SupplierName", "SupplierID", .Item(7, .CurrentCellAddress.Y).Value)
        End With
        btnClose.Text = "&Cancel"
        btnNew.Enabled = False
        btnSave.Enabled = True
    ...
    Private Sub cmbCustomerName_LostFocus(ByVal sender As Object, ByVal e As System.EventArgs) Handles cmbCustomerName.LostFocus
        If Len(cmbCustomerName.Text) > 0 And Len(cmbDRNumber.Text) > 0 Then
    ...
    Private Sub cmbCustomerNamer_SelectedValueChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles cmbCustomerName.SelectedValueChanged
        Call getCustomerIDFromComboBOx()
        Call LoadPOToCombo(SupplierID)
    End Sub


    Private Sub chkEnableAddDeliveryToDR_MouseClick(ByVal sender As Object, ByVal e As System.Windows.Forms.MouseEventArgs) Handles chkEnableAddStocksForRelease.MouseClick
        If chkEnableAddStocksForRelease.CheckState = CheckState.Checked Then
    ...
    Private Sub cmbPO_SelectedIndexChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles cmbDRNumber.SelectedIndexChanged
        btnAddNewSupplier_Click(sender, e)
    End Sub
    ...
    Private Sub btnAddToDRList_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnAddToDRList.Click
        ...
        If Len(txtProductBarcode.Text) > 0 And Len(txtqtyReceived.Text) > 0 Then
            Dim Barcode As String, ProductDesciption As String, UnitName As String, QTYordered As Double, amount As Double, Disc As Double, Transtime As String, unitprice As Double

            Barcode = txtProductBarcode.Text
            ProductDesciption = txtProductDescription.Text
            UnitName = cmbUnitType.Text
            QTYordered = txtqtyReceived.Text
            unitprice = cmbUnitPrice.Text
            Batch_Number = txtDRNumber.Text
            amount = unitprice * QTYordered


            'get qtyOnHand from dbase
            Dim QTYONHAND As Long = getFIELD_VALUE_FROM_DBASE("tblProduct", "QtyOnHand", "ProductID_Barcode", txtProductBarcode.Text)

            Dim totalQtyOrdered As Long = evaluateQTY_to_Release(txtProductBarcode.Text) + Convert.ToDouble(txtqtyReceived.Text)

            If totalQtyOrdered > Convert.ToDouble(txtQTYOnHand.Text) Then
                If MsgBox("The Quantity to release is Greater than Quantity On Hand, there are only " & QTYONHAND - evaluateQTY_to_Release(txtProductBarcode.Text) & " available, Please re-check inventory and try again!" + Chr(13) + Chr(13) + "PROCEED ANYWAY?", MsgBoxStyle.Critical + MsgBoxStyle.YesNo, "") = MsgBoxResult.No Then

                    Exit Sub
                Else
                    txtQTYOnHand.Text = Convert.ToDouble(txtQTYOnHand.Text) - totalQtyOrdered
                End If
            Else
                txtQTYOnHand.Text = Convert.ToDouble(txtQTYOnHand.Text) - totalQtyOrdered
            End If

            Dim row As String() = New String() {Barcode, ProductDesciption, totalQtyOrdered, UnitName, unitprice, amount, "", Batch_Number} 'FormatCurrency(unitprice, 2)
            DataGridView2.Rows.Add(row)
    ...
    Private Function evaluateQTY_to_Release(ByVal bcode As String) As Long
        Dim qtyToReleaseed As Long = 0
        With DataGridView2
            For xDatagridRows As Integer = 0 To .Rows.Count - 2
                If bcode = .Rows(xDatagridRows).Cells(0).Value Then
                    Dim xxyzzz As String
                    'update tblProduct QtyOnHand,QTYInn,SupplierID
                    xxyzzz = .Rows(xDatagridRows).Cells(0).Value

                    qtyToReleaseed = qtyToReleaseed + IIf(.Rows(xDatagridRows).Cells(2).Value = "", 0, .Rows(xDatagridRows).Cells(2).Value)
                End If
            Next
        End With
        Return qtyToReleaseed
    End Function
    ...
    Private Sub txtDRNumber_LostFocus(ByVal sender As Object, ByVal e As System.EventArgs)
        Dim DR_Ref As String = txtDRNumber.Text
    ...
    Private Sub txtDRNumber_TextChanged(ByVal sender As System.Object, ByVal e As System.EventArgs)
